...
### BenSim
* A Python package for measuring the semantic similarity among sentences in Bengali language.
* Users will provide a reference sentence and a list of target sentences as input; and optionally the similarity assessment approach _(Default: Cosine Similarity)_ and the maximum sequence length _(Default: 512)_.
* BenSim will extract the contextual embeddings of the reference sentence and target sentences through a pre-trained BERT model.
* The similarities will be measured between each of the sentence pairs by applying either Euclidean distance or Cosine similarity (based on the input parameter).
...
* Details usage can be found [here](https://github.com/AbuUbaida/BenSim "GitHub").
